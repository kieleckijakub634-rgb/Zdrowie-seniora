        ...
        try {
          ...
          if (!error && data && data.user) {
            ...
            if (!logRes.error && logRes.data && logRes.data.user) {
              ...
              const loginError = (logRes.error?.message || '').toLowerCase();
              if (loginError.includes('confirm') || loginError.includes('potwierd')) {
                await window.supabaseClient.auth.resend({
                  type: 'signup',
                  email: pEmail,
                  options: { emailRedirectTo: getSupabaseAuthRedirectUrl() }
                }).catch(e => console.error("Resend signup verification error:", e));
                showSignupVerification(pEmail);
                return;
              }
