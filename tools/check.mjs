...
const polityka = read('www/pages/polityka.html');
...
assert(![index, home, facebook, polityka, regulamin, router, app].some(text => /testow.*stripe|stripe.*testow|sandbox|rzeczywiste środki/i.test(text)), 'Interfejs nadal ujawnia testowy tryb Stripe.');
...
assert(index.includes('id="inp-password"') && auth.includes('.auth.signUp('), 'Rejestracja nie tworzy konta przed płatnością.');
assert(auth.includes('verifyVerificationCode') && auth.includes("type: 'signup'"), 'Brakuje potwierdzenia e-maila przed Stripe.');
assert(!auth.includes("functions.invoke('registration-checkout'"), 'Formularz nadal korzysta z procesu tworzenia konta po płatności.');
assert(router.includes('isSignupConfirm') && router.includes('startStripeCheckout(session.user'), 'Link potwierdzający e-mail nie prowadzi do Stripe.');
assert(index.includes('Konto i adres e-mail zostały już potwierdzone'), 'Komunikat po płatności nie odpowiada kolejności rejestracja → e-mail → Stripe.');
